	...
	public function lancamentos(){
        ...
        if($periodo == 'todos'){
            ...
            if($situacao == 'previsto'){
                $where = 'data_Emissao > "'.date('Y-m-d').'" AND status = "0"'; 
            }
            else{
            ...
            if($periodo == null || $periodo == 'dia'){
                ...
                if($periodo == 'semana'){
                    ...
                    if(! isset($situacao) || $situacao == 'todos'){
                        ...
                        $where = 'data_Emissao BETWEEN "'.$semana[0].'" AND "'.$semana[1].'"'; 

                    }
                    else{
                        if($situacao == 'previsto'){
                            $where = 'data_Emissao BETWEEN "'.date('Y-m-d').'" AND "'.$semana[1].'" AND status = "0"'; 
                        }
                        else{
                            if($situacao == 'atrasado'){
                                $where = 'data_Emissao BETWEEN "'.$semana[0].'" AND "'.date('Y-m-d').'" AND status = "0"'; 
                            }
                            else{
                                if($situacao == 'realizado'){
                                    $where = 'data_Emissao BETWEEN "'.$semana[0].'" AND "'.$semana[1].'" AND status = "1"';
                                }
                                else{
                                    $where = 'data_Emissao BETWEEN "'.$semana[0].'" AND "'.$semana[1].'" AND status = "0"';
                                }
                            }
                        }
                    }
                    ...
                    if($periodo == 'mes'){
                        ...
                        if(! isset($situacao) || $situacao == 'todos'){
                            ...
                            $where = 'data_Pag BETWEEN "'.$mes[0].'" AND "'.$mes[1].'"'; 

                        }
                        else{
                            if($situacao == 'previsto'){
                                $where = 'data_Emissao BETWEEN "'.date('Y-m-d').'" AND "'.$mes[1].'" AND status = "0"'; 
                            }
                            else{
                                if($situacao == 'atrasado'){
                                    $where = 'data_Emissao BETWEEN "'.$mes[0].'" AND "'.date('Y-m-d').'" AND status = "0"'; 
                                }
                                else{
                                    if($situacao == 'realizado'){
                                        $where = 'data_Emissao BETWEEN "'.$mes[0].'" AND "'.$mes[1].'" AND status = "1"';    
                                    }
                                    else{
                                        $where = 'data_Emissao BETWEEN "'.$mes[0].'" AND "'.$mes[1].'" AND status = "0"';
                                    }
                    ...
                    // busca lançamentos do ano
                        ...
                        if(! isset($situacao) || $situacao == 'todos'){
                            ...
                            $where = 'data_Emissao BETWEEN "'.$ano[0].'" AND "'.$ano[1].'"';

                        }
                        else{
                            if($situacao == 'previsto'){
                                $where = 'data_Emissao BETWEEN "'.date('Y-m-d').'" AND "'.$ano[1].'" AND status = "0"'; 
                            }
                            else{
                                if($situacao == 'atrasado'){
                                    $where = 'data_Emissao BETWEEN "'.$ano[0].'" AND "'.date('Y-m-d').'" AND status = "0"'; 
                                }
                                else{
                                    if($situacao == 'realizado'){
                                        $where = 'data_Emissao BETWEEN "'.$ano[0].'" AND "'.$ano[1].'" AND status = "1"';        
                                    }
                                    else{
                                        $where = 'data_Emissao BETWEEN "'.$ano[0].'" AND "'.$ano[1].'" AND status = "0"';
                                    }
        ...
        $config['total_rows'] = $this->financeiro_model->count('reconc_bank',$where);
        $config['per_page'] = 20;
        ...
        $this->data['result_caixas'] = $this->financeiro_model->get2('caixas');   
		$this->data['results'] = $this->financeiro_model->get('reconc_bank','id_reconc,id_aenp,data_Emissao,data_Pag,status,operador',$where,$config['per_page'],$this->input->get('per_page'));
         ...
         //   $vencimento = $this->input->post('vencimento');
            ...
            $data = array(
              ...
              //  'valor' => $this->input->post('valor'),
                //'data_vencimento' => $vencimento,
                'data_Pag' => $pagamento,
                'status' => $this->input->post('pago') ? : 0,
              //  'cliente_fornecedor' => $this->input->post('fornecedor'),
              //  'forma_pgto' => $this->input->post('formaPgto'),
                //'tipo' => $this->input->post('tipo')
            );

            if ($this->financeiro_model->edit('reconc_bank',$data,'id_reconc',$this->input->post('id')) == TRUE) {
                if($this->input->post('pago') !==null)
                    $this->session->set_flashdata('success','<strong>Cheque Baixado</strong> com sucesso!');
                else $this->session->set_flashdata('error','Cheque foi Atualizado para pendente!');
                redirect($urlAtual);
            } else {
