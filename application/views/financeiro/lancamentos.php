	...
	<form action="<?php echo current_url(); ?>" method="get" >
		...
		<div class="span4">
			...
			<select name="situacao" class="span12">
				...
				<option value="realizado" <?php if($situacao == 'realizado'){ echo 'selected';} ?>>Baixados</option>
...
<?php if($this->permission->checkPermission($this->session->userdata('permissao'),'aLancamento')){ ?>
  <div class="span5" style="margin-left: 0"><!--
      <a href="#modalReceita" data-toggle="modal" role="button" class="btn btn-success tip-bottom" title="Cadastrar nova receita"><i class="icon-plus icon-white"></i> Baixa de </a>  
...
<table class="table table-bordered " id="divLancamentos">
    <thead>
        <tr style="backgroud-color: #2D335B">
            <th>Conta</th>
            <th>Lançamento | Cheque</th>
            <th>Data Emissão</th>
            <th>Data baixa (ou previsão)</th>
            <th>Status</th>
            <th>Valor</th>
            <th></th>
        </tr>
    ...
    <tbody> 
        ...
        foreach ($results as $r) {
            $vencimento = date(('d/m/Y'),strtotime($r->data_Emissao));
            $paganto = date(('d/m/Y'),strtotime($r->data_Pag));
            foreach ($result_caixas as $rcx) {
            if($rcx->id_caixa == $r->conta)$contA = $rcx->nome_caixa;
            }
            if($r->status == 0){$status = 'Pendente'; $cor ="red";}else{ $status = 'Pago';  $cor ="green";};
            echo '<tr>'; 
            echo '<td>'.$contA.'</td>'; 
            echo '<td>'.$r->id_aenp.' | '.$r->num_Doc_Banco.'</td>';
            echo '<td>'.$vencimento.'</td>'; 
            echo '<td>'.$paganto.'</td>';  
            echo '<td><span class="badge" style="background-color: '.$cor.'; border-color: #8A9B0F">'.$status.'</span></td>'; 
            echo '<td align="right">'.number_format($r->valorFin,2,',','.').'</td>';
       //     echo '<td> R$ '.number_format($r->valor,2,',','.').'</td>';
            
            echo '<td>';
            if($this->permission->checkPermission($this->session->userdata('permissao'),'eLancamento')){
                echo '<a href="#modalEditar" style="margin-right: 1%" data-toggle="modal" role="button" 
                idLancamento="'.$r->id_reconc.'"  
                vencimento="'.$vencimento.'" 
                pagamento="'.$paganto.'" 
                baixado="'.$r->status.'" 
                descricao="'.$r->descricao.'" 
                cliente="'.$r->historico.'" 
                valor="'.number_format($r->valorFin,2,',','.').'" 
                class="btn btn-info tip-top editar" title="Baixa de Cheque">
...
<div id="modalReceita" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  <div class="modal-header">
    ...
    <h3 id="myModalLabel">CiscoF - Adicionar Receita</h3>
  </div>
  ...
  <div class="modal-body">
    	...
    	<div class="span12" style="margin-left: 0"> 
    		...
    		<div id="divPagamento" class="span8" style=" display: none">
	    		<div class="span6">
	    			<label for="pagamento">Data da Baixa</label>
...
<div id="modalEditar" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  <div class="modal-header">
    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
    <h3 id="myModalLabel">CISCOF - Baixa em cheque</h3>
  </div>
  <div class="modal-body">
      <div class="span12 alert alert-info" style="margin-left: 0"> Utilize "Foi Pago" para alterar a situação do cheque.</div>
      <div class="span12" style="margin-left: 0"> 
        <label for="descricao">Descrição</label>
        <input class="span12" id="descricaoEditar" type="text" name="descricao"  readonly  />
        <input id="urlAtualEditar" type="hidden" name="urlAtual" value=""  />
      </div>  
      <div class="span12" style="margin-left: 0"> 
        <div class="span12" style="margin-left: 0"> 
          <label for="fornecedor">Fornecedor / Empresa*</label>
          <input class="span12" id="fornecedorEditar" type="text" name="fornecedor"  readonly  />
        </div>
      </div>
      <div class="span12" style="margin-left: 0"> 
        <div class="span4" style="margin-left: 0">  
          ...
          <input class="span12 money"  type="text" name="valor" id="valorEditar"  readonly />
        </div>
        <div class="span4" >
          <label for="vencimento">Data Emissão*</label>
          <input class="span12 datepicker"   type="text" name="vencimento" id="vencimentoEditar"   readonly />
        </div>
        ...
        <div id="divPagamentoEditar" class="span8" style=" display: none">
          <div class="span6">
            ...
            <input class="span12 datepicker" id="pagamentoEditar" type="text" name="pagamento" />  
             ...
             <input class="span12 " name="formaPgto" id="formaPgtoEditar"  type="hidden"  value="1" />
        </div>        
      </div>
  </div>
  <div class="modal-footer">
